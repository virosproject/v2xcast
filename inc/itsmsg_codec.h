 ...
 */
extern int itsmsg_dealloc(ItsPduHeader *msg);
 ...
 * @retval -1 Failed.
 */
extern int itsmsg_encode_in_target_buf(uint8_t **buf, ItsPduHeader *msg, ITSMsgCodecErr *err, const ITSMsgConfig *cfg);
...
/**
 * Decode the UPER format data to pre-defined message structure for processingand and all memory usage will limited in targer memory buffer
 *
 * Because user has no way to determine the type of message after decoding,
 * the target memory buffer size need fine tune and
 * the memory of decoded message structure will limited in target buffer.
 ...
 * @retval -1 Failed.
 */
extern int itsmsg_decode_in_target_buf(ItsPduHeader **msg, const uint8_t *buf, int buf_len, ITSMsgCodecErr *err, const ITSMsgConfig *cfg);
